...
export class Manager {
  ...
  existsLocal() { // 判断本地是否已经存在lockFilename文件(即api-lock.json)
    // 改动处：config.name可能为空,而且lockFilename文件根本不在config.name，所以不用加这层即可
    return (
      fs.existsSync(path.join(this.currConfig.outDir, this.lockFilename)) ||
      _.some(this.allConfigs.map((config) => fs.existsSync(path.join(config.outDir, this.lockFilename)))) // 改动后代码
      // _.some(this.allConfigs.map((config) => fs.existsSync(path.join(config.outDir, config.name, this.lockFilename)))) // 原始代码
    );
  }
  ...
  async initRemoteDataSource(config = this.currConfig) {
    ...
    if (oldRemoteSource) {
      ...
      console.log(JSON.stringify(remoteDataSource));
